...
% ---------------------------- STATUS ----------------------------
\begin{status}
\textbf{\color{laranja}Estado desta versão do relatório (checkpoint de 04/09/2026).}
Este documento consolida a \textbf{especificação formal}, os \textbf{algoritmos e pseudocódigos},
a \textbf{prova de corretude}, a \textbf{análise no modelo RAM}, as \textbf{recorrências}, a
\textbf{metodologia experimental} e a \textbf{relação com RAG} --- partes que não dependem da
execução do pipeline. As Figuras~\ref{fig:crescimento} e~\ref{fig:arvore} são \textbf{curvas e
diagramas teóricos}, derivados das funções de complexidade, e não medições. As tabelas da
Seção~\ref{sec:resultados} (resultados empíricos) trazem a estrutura definitiva de colunas e estão
marcadas \emph{``a preencher após a execução''}; os números serão inseridos após rodar
\texttt{scripts/reproduzir\_tudo.py}, conforme o cronograma (demonstração inicial no checkpoint de
...
\begin{align*}
\mathrm{tf}(t,c) &= 1 + \ln f_{t,c} \quad (\text{se } f_{t,c}>0;\ \text{senão } 0), &
\mathrm{idf}(t) &= \ln\!\left(\frac{N+1}{\mathrm{df}(t)+1}\right)+1,\\[2pt]
\mathrm{score}(q,c) &= \sum_{t\in q} \mathrm{tf}(t,c)\cdot\mathrm{idf}(t). & &
\end{align*}
$f_{t,c}$ é a frequência do termo $t$ no chunk $c$ e $\mathrm{df}(t)$ é o número de chunks que
contêm $t$. Termos ausentes do vocabulário não contribuem. As formas suavizadas ($\ln f$ no tf e
$\ln\frac{N+1}{\mathrm{df}+1}+1$ no idf) são as mesmas do TF-IDF do scikit-learn, o que mantém limpa a
comparação com a Config.~4 e evita zerar um termo presente em todos os chunks. A \textbf{ordem total}
$\preceq$ é:
\[
...
\textbf{sobreposição de 32}, produzindo cerca de \textbf{330 chunks}. As configurações 128/16 e
...
funcao ConsultarIndice(q, vocab, postings, idf):
  acc <- mapa id -> score (vazio)
  para cada termo t em q faca            # m termos
    p <- BuscaBinaria(vocab, t)          # Theta(log V)
    se p != NAO_ENCONTRADO entao
      para cada (id, f) em postings[t] faca   # so os candidatos!
        acc[id] <- acc[id] + (1 + ln f) * idf[t]
  candidatos <- [ (s, id, chunk(id)) para (id, s) em acc ]
  retorne MergeSort(candidatos)          # ordena so os candidatos
\end{lstlisting}

\subsection{Seleção dos $k$ melhores (comum a todas)}
\begin{lstlisting}
funcao TopK(candidatos_ordenados, k):
  retorne os primeiros min(k, |candidatos_ordenados|) elementos
\end{lstlisting}
Como a lista já está totalmente ordenada por $\preceq$, os primeiros $\min(k,n')$ elementos são, por
definição, os $k$ mais relevantes (corolário na Seção~\ref{sec:corretude}).
...
% ATUALIZAR o número na versão congelada (quinta): 87 testes em 23/09; 118 na versão da Nadianne.
(87 testes na versão de 23/09/2026). Esta subseção consolida a análise de corretude conduzida por
Nadianne Galvão (\texttt{docs/CORRETUDE.md} e o documento de corretude, RAM, complexidade e
recorrências).
...
\midrule
$N$ & número de chunks ($\approx$330 na configuração principal)\\
...
$\sum_{i=1}^{C_q-1} i = C_q(C_q-1)/2$ no pior caso, com igual ordem de acessos e atribuições. Espaço
auxiliar $\Theta(1)$.
...
\midrule
Pontuação / candidatos & $\Theta(N m)$ & $\Theta(m\log V + P)$ & $\Theta(N m)$\\
Ordenação dos candidatos & $O(C_q^2)$ & $\Theta(C_q\log C_q)$ & $\Theta(C_q\log C_q)$\\
...
\midrule
Busca/pontuação linear & $\Theta(Nm)$ & $\Theta(Nm)$ & $\Theta(Nm)$ & $\Theta(C_q)$\\
...
Consulta via índice (C2) & $\Theta(m\log V + C_q\log C_q)$ & $\Theta(m\log V + N\log N)$ & $\Theta(m\log V + P + C_q\log C_q)$ & $\Theta(V+P_{\text{tot}})$\\
\bottomrule
\end{tabularx}
\end{table}
Onde $n=C_q$ é o número de candidatos a ordenar.
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{Insertion Sort} só é competitivo com $C_q$ pequeno ou lista quase ordenada; melhor caso
linear quando não há deslocamento.
\item \textbf{Merge Sort} tem custo $\Theta(n\log n)$ \textbf{garantido} nos três casos ---
escolha segura para a comparação.
\item \textbf{Consulta via índice (C2)} tem melhor caso com termos \textbf{raros} ($P$ e $C_q$
pequenos) e degenera ao custo de C3 quando $C_q\approx N$ (termo muito frequente). A hipótese de
esparsidade é o que separa C2 de C3 na prática.
\end{itemize}
...
\textbf{Origem dos termos:} uma única chamada recursiva sobre metade do vocabulário; $\Theta(1)$ é o
cálculo do meio e a comparação com \texttt{vocab[mid]}. Pelo Teorema Mestre, $a=1$, $b=2$,
$N^{\log_b a}=N^0=1=f(N)$: caso~2, dando $T(V)=\Theta(\log V)$.
...
Plano: \textbf{3 configurações $\times$ 3 tamanhos de corpus $\times$ 5 repetições $=$ 45 execuções
mensuráveis} (bem acima do mínimo de 12). Os três tamanhos vêm de \textbf{subamostragem
determinística} (semente fixa) de $\approx$25\%, 50\% e 100\% dos chunks; o experimento de
sensibilidade varia o chunking em 128/16, 256/32 e 512/64. Cada consulta roda para $k=5$ e $k=10$.
As \textbf{duas primeiras} execuções de cada cenário são descartadas como aquecimento; reporta-se a mediana das repetições com a dispersão.
...
\item \textbf{Precision@k} contra \texttt{data/qrels.csv}, quando há referência.
...
das bibliotecas, commit e a linha de comando completa. Consultas e qrels foram fixados \textbf{antes}
de observar resultados, evitando viés de seleção.

\subsection{Julgamentos de relevância}
As 30 consultas recebem julgamentos em escala 0, 1 e 2 por \textbf{dois avaliadores independentes},
com a adjudicação dos desacordos registrada em \texttt{data/qrels.csv}, o que dá a referência para o
Precision@k e permite reportar a concordância entre avaliadores.
...
\begin{status}
\textbf{Tabelas empíricas --- a preencher após a execução.} A estrutura de colunas abaixo é a
definitiva; os valores serão inseridos a partir de \texttt{experimentos/processados} após rodar
\texttt{scripts/rodar\_experimentos.py}. A previsão qualitativa (não medida) que os experimentos vão
testar está indicada em itálico após cada tabela.
\end{status}

\subsection{Tempo por etapa e configuração (mediana de $r$ repetições)}
\begin{table}[h]
\small\centering
\begin{tabularx}{\textwidth}{@{}llYYYYY@{}}
\toprule
\textbf{Config.} & \textbf{N} & \textbf{Ingestão} & \textbf{Ord./índice} & \textbf{Consulta} & \textbf{Total} & \textbf{Comparações}\\
\midrule
C1 & $\sim$330 & --- & --- & --- & --- & ---\\
C2 & $\sim$330 & --- & --- & --- & --- & ---\\
C3 & $\sim$330 & --- & --- & --- & --- & ---\\
...
\end{tabularx}
\end{table}
\emph{\small Previsão a testar: o tempo de ordenação de C1 cresce como $C_q^2$, superando C2/C3
conforme os candidatos aumentam; a consulta de C2 é a menor por tocar apenas os candidatos; as
comparações de C2/C3 ficam perto de $C_q\log_2 C_q$ e as de C1 perto de $C_q^2/2$ no pior caso.}
...
\toprule
\textbf{Config.} & \textbf{Precision@5} & \textbf{Precision@10} & \textbf{Taxa de vazios} & \textbf{Memória pico (MB)}\\
\midrule
C1 & --- & --- & --- & ---\\
C2 & --- & --- & --- & ---\\
C3 & --- & --- & --- & ---\\
...
\end{tabularx}
\end{table}
\emph{\small Previsão a testar: C1, C2 e C3 têm \textbf{Precision@k idêntica} (mesma pontuação e
mesma ordem total), confirmando que a diferença é de custo, não de qualidade; C2 consome mais memória
pelo índice; C4 pode divergir na Precision@k por usar normalização TF-IDF diferente.}

\subsection{Escalabilidade medida (a preencher)}
\begin{table}[h]
\small\centering
...
\end{tabularx}
\end{table}
\emph{\small Previsão a testar: a curva de C1 acompanha o crescimento super-linear do Insertion Sort;
C2 e C3 crescem de forma quase linear-logarítmica; a vantagem de C2 se amplia com $N$.}

% ============================ 13 ============================
\section{Discussão de escalabilidade}
Para o corpus atual ($\sim$330 chunks) as três configurações rodam em milissegundos; interessa a
\textbf{tendência}. À medida que o acervo cresce (dezenas de livros, $N$ na casa dos milhares):
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{C1} torna-se inviável: Insertion Sort cresce como $C_q^2$ e a varredura toca todos os
$N$ chunks por consulta.
\item \textbf{C2} desacopla o custo de consulta do tamanho total: depende da \textbf{seletividade dos
termos} ($P$, $C_q$) e de $\log V$, não de $N$. É a única que escala para acervos grandes --- ao
custo de memória para o índice ($\Theta(V+P_{\text{tot}})$).
\item \textbf{C3} mantém a ordenação em $\Theta(N\log N)$, mas ainda paga a varredura $\Theta(Nm)$.
\end{itemize}
Esse é o trade-off central da recuperação: gastar memória e indexação uma vez para tornar cada
consulta sublinear. Bibliotecas como FAISS levam a ideia ao caso vetorial denso; aqui a demonstramos
no caso lexical, com estruturas clássicas.
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{Construção.} A Precision@k depende de julgamentos subjetivos; mitigada com dois
avaliadores e adjudicação, mas imperfeita.
...
Toda saída de IA será conferida quanto a corretude, complexidade, casos de borda, estabilidade de
ordenação e desempate, e coerência entre análise e experimento (\S9 do enunciado). Código, prova ou
análise não verificados não entram no trabalho.\\
\bottomrule
\end{tabularx}
\end{table}
\begin{nota}
\textbf{Compromisso de uso crítico.} A prova da Seção~\ref{sec:corretude} será validada por testes de
estabilidade e desempate; os pseudocódigos, por implementação e testes unitários; e as previsões da
Seção~\ref{sec:resultados}, pela execução real.
\end{nota}
